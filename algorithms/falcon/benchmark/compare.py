...
def print_summary(traditional: dict, pqc: dict, e2e: dict | None) -> None:
    print("=== Falcon (falcon_512) Comparison Summary ===")
...
def save_plot_and_md(traditional: dict, pqc: dict, e2e: dict | None) -> tuple[Path, Path]:
    ...
    fig.suptitle("Falcon (falcon_512): Benchmark Comparison", fontsize=15, fontweight="bold")
    ...
    if has_ipfs:
        bar2(axs[1, 1], "IPFS Upload (s)",
             ["Sig upload", "PK upload"], [ipfs_sig_up, ipfs_pk_up], [green, green])
    else:
        axs[1, 1].axis("off")

    if has_e2e:
        bar2(axs[1, 2], "E2E Latency Breakdown (s)",
